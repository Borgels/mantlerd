...
func TestRunCheckInWithoutFollowUpWhenDisabled(t *testing.T) {
	var (
		mu       sync.Mutex
		requests []types.CheckinRequest
	)
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		response := map[string]any{
			"data": types.CheckinResponse{
				...
				DesiredConfig: types.DesiredConfig{
					Harnesses: []types.DesiredHarness{
						{ID: "h1", Name: "Harness 1", Type: "codex_cli"},
					},
				},
			},
		}
	...
	cfg := config.Config{ServerURL: server.URL, Token: "test-token", MachineID: "machine-1", Interval: 30 * time.Second, Insecure: true}
	runtimeManager := runtime.NewManager()
	trainerManager := trainer.NewManager()
	executor := commands.NewExecutor(runtimeManager, trainerManager, cfg, nil, nil)
	dispatcher := newCommandDispatcher(context.Background(), executor, cl, defaultLightCommandConcurrency)
	...
	runCheckIn(context.Background(), cfg, cl, runtimeManager, trainerManager, executor, outcomes, dispatcher, time.Now(), false)
	waitCtx, waitCancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer waitCancel()
	if !dispatcher.WaitForIdle(waitCtx) {
		t.Fatal("timed out waiting for command completion")
	...
	defer mu.Unlock()
	if len(requests) != 1 {
		t.Fatalf("expected exactly 1 checkin request when follow-up disabled, got %d", len(requests))
	}
}

func TestRunCheckInRateLimitedErrorReturnsBackoff(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/agent/checkin" {
			w.WriteHeader(http.StatusNotFound)
			return
	...
	result := runCheckIn(context.Background(), cfg, cl, runtimeManager, trainerManager, executor, outcomes, nil, time.Now(), true)
	if result.success {
		t.Fatal("expected checkin cycle to fail on 429")
	}
	if result.rateLimitBackoff != time.Second {
		t.Fatalf("expected 1s rate-limit backoff, got %v", result.rateLimitBackoff)
	}
	if result.activeOperations {
		t.Fatalf("expected no active operations on clean runtime, got activeOperations=%v", result.activeOperations)
	}
}
